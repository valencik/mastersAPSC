...
Data representation and classification are necessary steps to establish the structures within the database, ie. papers and fields or genres.
Cluster analysis is used to group data elements based on rules.
K-means clustering will be looked at in detail.
...
The distance metric is the function on a space that describes distance...
In text mining or document comparing we often use cosine similarity between term frequency vectors. (citation?)
Mahalanobis distance metric leads to hyperellopsoidal clusters.\citep{MaoJain1996}
%Itakura-Saito distance for vector quantization in speech processing (Linde et al., 1980)

The chosen number of clusters has a huge impact on the data partitions.
Some heuristics exist to aid in determining an optimal $k$. %TODO citation
...
\cos (d1,d1) = \frac{d1 \cdot d2 }{\left| d1 \right| \left| d2 \right|} = \frac{6.0}{\left| 2.645751311 \right| \left| 2.828427125 \right|} = 0.8017837257
\end{equation}
...
% - Selected technique: Digital Signal Processing (FPGAs)
\pagebreak
...
\\
%EXPAND - Reference above sections, what is neutrinoless double beta-decay?
...
%TREND DSP, triggering, commercial
\citep{Alberto200999}
